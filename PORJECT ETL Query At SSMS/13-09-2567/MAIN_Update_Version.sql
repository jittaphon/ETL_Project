...
Aging_Ar AS (
SELECT 
    SUM([Overdue_181-360]) + SUM([Overdue_360]) AS Total_Overdue,
    CASE 
        WHEN SUM([Overdue_181-360]) + SUM([Overdue_360]) > 180 THEN 'Red'
        ELSE 'Green'
    END AS [status]
FROM 
...
SELECT 
    ...
    END AS [status],
	FORMAT(DATEADD(MONTH, -1, GETDATE()), 'yyyy-MM') AS YearMonth,
